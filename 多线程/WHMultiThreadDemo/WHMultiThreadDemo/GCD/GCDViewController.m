...
- (void)communicationBetweenThread {
    ...
    // 异步
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        NSLog(@"Globle queue %@",[NSThread currentThread]);
...
- (void)testGroup {
    ...
    dispatch_group_async(group, dispatch_get_global_queue(0, 0), ^{
        NSLog(@"队列组1：有一个耗时操作完成！%@",[NSThread currentThread]);
    });
    
    dispatch_group_async(group, dispatch_get_global_queue(0, 0), ^{
        NSLog(@"队列组2：有一个耗时操作完成！%@",[NSThread currentThread]);
    });
    
    dispatch_group_notify(group, dispatch_get_main_queue(), ^{
        NSLog(@"队列组3：前面的耗时操作都完成了，回到主线程进行相关操作 %@",[NSThread currentThread]);
    });
}
